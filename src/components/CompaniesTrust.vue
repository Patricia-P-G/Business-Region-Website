<template>
  ...
  <section id="testimonials" class="relative w-full overflow-hidden bg-[linear-gradient(to_bottom,#F8F8F8_0%,#EEF0FF_55%,#9BA8F0_100%)] py-14 sm:py-20"
    aria-labelledby="partners-title" aria-describedby="partners-description" itemscope itemtype="https://schema.org/Organization">
    ...
    <div class="relative z-10 mx-auto max-w-400 px-5 xs:px-6 sm:px-8 lg:px-20">
      ...
      <div class="grid grid-cols-12 gap-2.5">
        ...
        <div class="col-span-12">
          ...
          <!-- Title -->
          <h2 id="partners-title" class="flex justify-center text-center text-xl font-bold uppercase text-primary xs:text-2xl md:text-3xl lg:justify-start lg:text-4xl"
            itemprop="name">
            Companies that trust us:
          </h2>

          <!-- Only for Screen Readers -->
          <p id="partners-description" class="sr-only">
            Customer testimonials from companies and professionals who trust Business Region for
            cross-border business collaboration, networking, and growth opportunities.
          </p>
        </div>

        <!-- Underline -->
        <div class="col-span-12" aria-hidden="true">
        ...
        <div class="col-span-12 mt-10 md:mt-20">
          ...
          <div class="flex flex-col gap-8 md:hidden" aria-label="Customer testimonials">
            ...
            <article v-for="person in testimonials" :key="`mobile-${person.name}`" class="relative w-full overflow-visible rounded-[14px] bg-tertinary
              ...
              <div class="flex items-center gap-4">
                ...
                <!-- Image -->
                <img :src="person.image" :alt="`Portrait of ${person.name}, ${person.position}`" class="h-18 w-18 shrink-0 rounded-full object-cover max-[420px]:h-15.5 max-[420px]:w-15.5"
              ...
              <!-- Quote -->
              <blockquote class="relative z-10 mt-8 pl-6 pr-2.5 text-sm leading-[1.65] text-white max-[420px]:mt-7 max-[420px]:pl-4.5 max-[420px]:pr-1.5 max-[420px]:text-[13px] max-[420px]:leading-[1.6]"
                itemprop="reviewBody">
                {{ person.quote }}
              </blockquote>
          ...
          <div class="hidden grid-cols-12 items-center gap-2.5 md:grid max-[1399px]:gap-2 max-[1049px]:grid-cols-1 max-[1049px]:gap-6"
            aria-label="Customer testimonials carousel">

            <!-- Card Wrapper -->
            <article v-for="(person, index) in testimonials" :key="`desktop-${person.name}`" class="col-span-4 w-full max-[1049px]:col-span-1"
              itemscope itemtype="https://schema.org/Review" itemprop="review">

              <!-- Clickable Card -->
              <button type="button" class="relative w-full overflow-visible rounded-[14px] text-left transition-all duration-[0.45s] ease-in-out focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#6474bd] motion-reduce:transition-none"
                ...
                <div class="relative min-h-105 rounded-[14px] p-10 text-white shadow-[0_0_18px_rgba(121,139,210,0.65)] transition-all duration-[0.45s] ease-in-out motion-reduce:transition-none motion-reduce:transform-none max-[1399px]:min-h-90 max-[1399px]:p-7
                  ...
                  <div class="flex items-center gap-5">
                    ...
                    <!-- Image -->
                    <img :src="person.image" :alt="`Portrait of ${person.name}, ${person.position}`"
                      class="h-18 w-18 shrink-0 rounded-full object-cover max-[1399px]:h-14.5 max-[1399px]:w-14.5 max-[1199px]:h-12.5 max-[1199px]:w-12.5 max-[1049px]:h-14.5 max-[1049px]:w-14.5"
                      width="72" height="72" loading="lazy" decoding="async"
                    />
                    ...
                    <div class="min-w-0">
                      ...
                      <!-- Name -->
                      <h3 class="text-xl font-semibold max-[1399px]:text-[15px] max-[1199px]:text-[13px] max-[1049px]:text-base" itemprop="author"
                        itemscope itemtype="https://schema.org/Person">
                        <span itemprop="name">{{ person.name }}</span>
                      </h3>

                      <!-- Position -->
                      <p class="mt-1 text-xs italic text-white/70 max-[1399px]:text-[10px] max-[1199px]:text-[9px] max-[1049px]:text-[11px]">
                        {{ person.position }}
                      </p>
                  ...
                  <!-- Quote -->
                  <blockquote class="relative z-10 mt-13 pl-12 pr-9.5 text-[15px] leading-[1.7] max-[1399px]:mt-9 max-[1399px]:pl-8 max-[1399px]:pr-5.5
                    max-[1399px]:text-[13px] max-[1399px]:leading-[1.6] max-[1199px]:mt-7 max-[1199px]:pl-6.5 max-[1199px]:pr-3.5 max-[1199px]:text-xs
                    max-[1199px]:leading-[1.55] max-[1049px]:mt-7 max-[1049px]:pl-6 max-[1049px]:pr-4.5 max-[1049px]:text-sm max-[1049px]:leading-[1.6]"
                    itemprop="reviewBody">
                    {{ person.quote }}
                  </blockquote>
